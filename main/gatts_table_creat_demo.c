...
static const char write_ascii_flag[] = "Write the ascii value \"yoRAL\" here";
...
static const esp_gatts_attr_db_t gatt_db[HRS_IDX_NB] =
{
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_FLAG_MD5, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_DEMO_CHAR_VAL_LEN_MAX, sizeof("First 20 Chars of MD5 of Device Name")-1, (uint8_t *)"First 20 Chars of MD5 of Device Name"}},
...
// Modified `set_score` function to update OLED display with score and progress
static void set_score()
{
    // Test if the function is called
    ESP_LOGI(GATTS_TABLE_TAG, "Updating OLED with score and progress...");
    ...
    // Debug log to check if the score is calculated correctly
    ESP_LOGI(GATTS_TABLE_TAG, "Calculated score: %d", score);

    // Clear the screen and display the title
    ssd1306_clear_screen(&dev, false);
    ssd1306_display_text(&dev, 0, "RAL BLE CTF", 11, false);  // Line 0: Title

    // Add a delay to give the OLED time to process the first command
    vTaskDelay(500 / portTICK_PERIOD_MS);

    // Display score
    char score_text[20];
    sprintf(score_text, "Score: %d/20", score);
    ssd1306_display_text(&dev, 2, score_text, strlen(score_text), false);  // Line 2: Score

    // Calculate and display progress
    int progress = (score * 100) / 20;
    char progress_text[20];
    sprintf(progress_text, "Progress: %d%%", progress);
    ssd1306_display_text(&dev, 4, progress_text, strlen(progress_text), false);  // Line 4: Progress

    // Debug log to check what is being displayed
    ESP_LOGI(GATTS_TABLE_TAG, "Displaying score: %s, progress: %s", score_text, progress_text);

    // Update the BLE characteristic for the score
            ...
            if (!param->write.is_prep){
                ...
                if (blectf_handle_table[IDX_CHAR_FLAG]+1 == param->write.handle)
                {
                    ...
                    if (strcmp(writeData,"5cd56d74049ae40f442e") == 0){
                        //First 20 Chars of md5 of device name
                        flag_state[3] = 'T';
                    }
                    ...
                    if (strcmp(writeData,"7ae3354275e1d0503d9e") == 0){
                        //final flag
                        flag_state[19] = 'T';
                    }
...
void app_main()
{
    ...
    i2c_master_init(&dev, GPIO_NUM_21, GPIO_NUM_22, -1); // SDA, SCL, RESET (-1 for no RESET)
    ssd1306_init(&dev, 128, 64);
    // Clear the screen and display the initial title
    ssd1306_clear_screen(&dev, false);
    ssd1306_display_text(&dev, 0, "RAL BLE CTF", 11, false);

    //uint8_t new_mac[8] = {0xDE,0xAD,0xBE,0xEF,0xBE,0xEF};
    //esp_base_mac_addr_set(new_mac);

    // Add a small delay to allow the initial text to be displayed
    vTaskDelay(500 / portTICK_PERIOD_MS);

    // Call set_score to display score and progress initially
    set_score();
